...
def gibbs_method(r1, r2, r3, mu: float = 398600.4418):
    """
...
def lambert(r1, r2, dt: float, mu: float = 398600.4418):
    """
    ...
    z = (
        np.dot(r1, r2)
        / np.sqrt(np.linalg.norm(r1) * np.linalg.norm(r2))
        * np.sqrt(mu)
        * x0
    )
    ...
    g = B / np.sqrt(mu)
    fdot = np.sqrt(mu) / (np.linalg.norm(r1) * np.linalg.norm(r2)) * (
        z * np.sin(np.sqrt(mu) * x0) - np.sqrt(mu) * x0
    ) * (1 - np.cos(np.sqrt(mu) * x0)) + (
        1 - np.cos(np.sqrt(mu) * x0)
    ) / np.linalg.norm(
        r2
    )
